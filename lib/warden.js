...
class Warden {
    ...
    getTriggers({ entity, action, data, row, rows, before }) {
        const action2 = Warden.ActionAlias[action];
        assert_1.default(action2);
        const key = `${entity}-${action2}`;
    ...
    async doTrigger({ trigger, txn, row, rows, data }, context) {
        ...
        if (triggerEntity) {
            ...
            const triggeredRows = await Object.getPrototypeOf(that).find.call(this, {
    ...
    async doTriggerAgain({ trigger, row, rows, data, txn }, context) {
        ...
        if (volatile === 'makeSure') {
            ...
            */
            ...
            await Object.getPrototypeOf(that).update.call(this, {
                entity,
                data: updateData,
                row,
                txn,
    ...
    async execTriggers({ triggers, row, rows, data, txn, context }) {
    ...
    async patrol(interval = 60000, context) {
        ...
        const promises = entities.map(entity => async () => {
            ...
            try {
                ...
                const rows = await Object.getPrototypeOf(that).find.call(this, {
                    entity,
                    txn,
                    projection: {
                        id: 1,

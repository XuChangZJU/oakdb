...
var SqlTranslator = /** @class */ (function (_super) {
        ...
        var checkInProjection = function (sortNode, projectionNode, entity, path) {
            ...
            attrs.forEach(function (attr) {
                if (attr.toLocaleLowerCase().startsWith('$fncall')) {
                    var $attrs = sortNode[attr].$attrs;
                    if ($attrs) {
                        $attrs.forEach(function (ele) {
                            if (!projectionNode.hasOwnProperty(ele)) {
                                throw errorCode_1.ErrorCode.createError(errorCode_1.ErrorCode.sortAttrUnexisted, "sort attribute " + path + "/" + ele + " unexisted in projection", {
                                    path: path,
                                    attr: ele,
                                });
                            }
    ...
    SqlTranslator.prototype.translateSort = function (entity, sort, aliasDict) {
        var _this = this;
        var schema = this.schema;
        var translateInner = function (entity2, sortAttr, path, prefix) {
            var attr = Object.keys(sortAttr)[0];
            var alias = aliasDict[path];
            var attributes = schema[entity2].attributes;
            if (attr.toLocaleLowerCase().startsWith('$fncall')) {
                return _this.translateFnCall(sortAttr[attr], alias, path.slice(2).replace(/\//g, '.'));
            }
                ...
                return translateInner(ref, sortAttr[attr], "" + path + attr + "/", prefix2);
            }
        };
        var sortText = '';

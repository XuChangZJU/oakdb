...
class SqlTranslator extends Translator_1.Translator {
    ...
    getDefaultProjection(entity, noExpand) {
        const projection = {};
        const { schema } = this;
            ...
            if (!attr.match(/\$\$[\d|\D]+\$\$$/)) { // omit metadata by default
                ...
                if (type === 'ref') {
                    if (!noExpand) {
                        ...
                        const projection2 = this.getDefaultProjection(ref, true);
                        lodash_1.assign(projection, {
                            [attr]: projection2,
                        });
                    }
                }
            ...
            Object.keys(node).forEach((op) => {
                if (Operator_1.LogicOperators.includes(op)) {
                    ...
                    if (!aliasDict.hasOwnProperty(pathAttr)) {
                        ...
                        from += ` left join \`${getStorageName(ref)}\` \`${alias2}\` on \`${alias}\`.\`${op}Id\` = \`${alias2}\`.\`id\``;
                    }
                    else {
                        alias2 = aliasDict[pathAttr];
                    }
